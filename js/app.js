...
function addNewPendingItem(label) {
  ...
  const trashButton = document.querySelector('.pending-items>li>.btn-delete-item');
  trashButton.addEventListener('click', (event) => {
    removeCurrentTargetParentElement(event);
  });
  const checkBox = document.querySelector('.pending-items>li>.checkbox');
...
function addNewCompletedItem(label) {
  ...
  const trashButton = document.querySelector('.completed-items>li>.btn-delete-item');
  trashButton.addEventListener('click', (event) => {
    removeCurrentTargetParentElement(event);
  });
  const checkBox = document.querySelector('.completed-items>li>.checkbox');
  checkBox.addEventListener('change', (event) => {
...
function moveToCompleted(event) {
  removeCurrentTargetParentElement(event);
  const label = event.currentTarget.parentElement.childNodes[2].textContent;
...
function completedNote() {
  ...
  const numberOfPendingItems = document.querySelectorAll('.pending').length;
  const percentage = Math.round(numberOfCompletedItems / (numberOfCompletedItems + numberOfPendingItems) * 100);
